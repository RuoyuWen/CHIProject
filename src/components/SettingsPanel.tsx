...
export function SettingsPanel({ onSettingsComplete }: SettingsPanelProps) {
  ...
  const [llmBPrompt, setLlmBPrompt] = useState(() => localStorage.getItem('llm_b_prompt') || 'You are "Module B" (Rendering/Interaction). Input is JSON from Module A. Output only 1-2 Chinese sentences for users. Maintain tactful, inspiring tone with user agency feeling while following strategies.');
